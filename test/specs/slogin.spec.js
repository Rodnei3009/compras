define([ 'require', 'lodash', 'angular-mocks', 'mocks'
    , '$App/slogin'
    , '$App/fhelper'
    , '$App/frest'
    , '$App/fconfig'
    ...
    describe("Social Login test", function () {
        ...
        function login() {
            ...
            httpBackend.expectGET(expectedLoginURL, expectHeaders(STANDARD_HEADERS)).respond(function() {
        ...
        function withoutDbIdTest(method, url, data, loginMethod) {
            ...
            loginMethod().then(function(result){
                    fail("Expected fail because of dbId is required, but get a success: " + JSON.stringify(result));
                },
                function(error){
        ...
        beforeEach(inject(function (_$rootScope_, _$location_, _$injector_, _$parse_, _$q_, _$timeout_, _$http_, _$httpBackend_) {
            Apperyio = require( '$App/fhelper' );
            Apperyio = Apperyio( _$rootScope_, _$location_, _$injector_, _$q_, _$parse_, _$timeout_ );
        ...
        function createUserTest(dbId) {
            httpBackend.expectPOST(usersUrl, loginBody, expectHeaders(STANDARD_HEADERS)).respond(function() {
        ...
        function updateUserTest(dbId) {
            login();
            httpBackend.expectPUT(updateUserUrl.replace("<user_id>", mocks.Login.user._id), loginBody, expectHeaders(HEADERS_WITH_TOKEN))
                .respond(function() {
                    return [200, mocks.Login.user, {}];
                });
        ...
        it("Update user without dbId", function () {
            ...
            withoutDbIdTest("PUT", updateUserUrl.replace("<user_id>", null), loginBody, updateUser);
        });

        function checkLoginTest(dbId) {
            login();
            httpBackend.expectGET(checkLoginUrl, expectHeaders(HEADERS_WITH_TOKEN)).respond(function() {
                return [200, mocks.Login.user, {}];
            });
        ...
        function userOauthLogoutTest(dbId) {
            login();
            httpBackend.expectDELETE(oauthLogout.replace("<provider>", providers.facebook.id), expectHeaders(HEADERS_WITH_TOKEN)).respond(function() {
                return [200, mocks.Login.user, {}];
            });
        ...
        it("User logout oauth without dbId", function () {
            ...
            withoutDbIdTest("DELETE", oauthLogout.replace("<provider>", providers.facebook.id), undefined, logoutOauth);
        });
        ...
        function mockForSocialLogin(windowOpenUrl, callbackURL) {
            ...
            window.open = function(url, windowName, windowFeatures) {
                ...
                expect(url).toEqual(windowOpenUrl);
        ...
        function loginTwitterTest(dbId, headers) {
            ...
            mockForSocialLogin(
                ...
                callback + "?oauth_token=" + mocks.Login.oauthToken + "&oauth_verifier=" + mocks.Login.oauthVerifier
            );
            ...
            httpBackend.expectGET(expectedTokenURL, expectHeaders(STANDARD_HEADERS)).respond(function() {
                return [200, {token: mocks.Login.oauthRequestToken}, {}];
            });
            ...
            httpBackend.expectPOST(oauthLoginUrl, expectedBody, expectHeaders(headers)).respond(function() {
                return [200, mocks.Login.user, {}];
            });
        ...
        function loginOauth2Test(loginMethod, provider, scope) {
            ...
            mockForSocialLogin(
                provider.baseUrl + "?client_id=" + clientId + "&redirect_uri=" + callback + "&scope=" + scope + "&response_type=code",
            ...
            httpBackend.expectPOST(oauthLoginUrl, expectedBody, expectHeaders(STANDARD_HEADERS)).respond(function() {
                return [200, mocks.Login.user, {}];
            });
        ...
        it("Login facebook without dbId", function () {
            ...
            mockForSocialLogin(
                providers.facebook.baseUrl + "?client_id=" + clientId + "&redirect_uri=" + callback + "&scope=" + "" + "&response_type=code",
                callback + "?code=" + mocks.Login.code
            );
            var expectedBody = {
                "verifier": mocks.Login.code,
        ...
        it("Login google without setDefaultDB", function () {
            var loginGoogle = function() {
                return SLogin.loginGoogle(clientId, callback, mocks.Login.dbId);
            };
            loginOauth2Test(loginGoogle, providers.google, "profile");
        });
        ...
        it("Login google with setDefaultDB", function () {
            var loginGoogle = function() {
                return SLogin.loginGoogle(clientId, callback);
            };
            SLogin.setDefaultDB(mocks.Login.dbId);
            loginOauth2Test(loginGoogle, providers.google, "profile");
            SLogin.setDefaultDB(undefined);
        ...
        it("Login google without dbId", function () {
            ...
            mockForSocialLogin(
                providers.google.baseUrl + "?client_id=" + clientId + "&redirect_uri=" + callback + "&scope=" + "profile" + "&response_type=code",
                callback + "?code=" + mocks.Login.code
            );
            var expectedBody = {
                "verifier": mocks.Login.code,
